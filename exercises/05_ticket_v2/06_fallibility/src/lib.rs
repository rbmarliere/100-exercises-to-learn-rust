...
impl Ticket {
    pub fn new(title: String, description: String, status: Status) -> Result<Ticket, String> {
        if title.is_empty() {
            return Err::<Ticket, String>("Title cannot be empty".into());
        }
        if title.len() > 50 {
            return Err::<Ticket, String>("Title cannot be longer than 50 bytes".into());
        }
        if description.is_empty() {
            return Err::<Ticket, String>("Description cannot be empty".into());
        }
        if description.len() > 500 {
            return Err::<Ticket, String>("Description cannot be longer than 500 bytes".into());
        }
